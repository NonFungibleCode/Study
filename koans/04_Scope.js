describe('scope 대해서 학습합니다.', function () {
  ...
  it('함수 선언식(declaration)과 함수 표현식(expression)의 차이를 확인합니다.', function () {
    ...
    expect(typeof funcDeclared).to.equal(typeof funcDeclared);
    expect(typeof funcExpressed).to.equal(typeof funcExpressed);
  ...
  it('lexical scope에 대해서 확인합니다.', function () {
    ...
    expect(shadowParameter('Parameter')).to.equal('Parameter');
  ...
  it('lexical scope와 closure에 대해 다시 확인합니다.', function () {
    ...
    // innerFn은 위에서 선언된 const innerFn을 가리키고, 뒤의 파란색 ()는 outerFn()이 할당된 변수 innerFn의 내부 함수를 가리키는 것! //
  });
});
